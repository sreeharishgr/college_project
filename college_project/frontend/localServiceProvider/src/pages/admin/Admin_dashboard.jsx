...
const theme = createTheme({
  palette: {
    mode: "light",
    primary: { main: "#3b5bfd" },
    text: { primary: "#1a1f36", secondary: "#5f6b85" },
    ...
    <Card>
      <CardContent>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography variant="body2">Activity Overview</Typography>
          <Chip label="Monthly" variant="outlined" />
        </Stack>
        <Box sx={{ mt: 2, height: isMobile ? 220 : 300 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={activity}>
              <CartesianGrid vertical={false} stroke={alpha("#000", 0.1)} />
              <XAxis dataKey="name" tickLine={false} axisLine={false} tick={{ fontSize: 12 }} />
              <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 12 }} />
              <ReTooltip />
              <Bar dataKey="uv" radius={[6, 6, 0, 0]} fill={theme.palette.primary.main} />
            </BarChart>
          </ResponsiveContainer>
        </Box>
      </CardContent>
...
export default function LocalServiceFindDashboard() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/admin/getDashboard`);
  ...
  const drawer = (
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
      ...
      <Divider />
      <Typography align="center" sx={{ p: 1, fontSize: 12, color: "gray" }}>
        Admin Panel
      </Typography>
    </Box>
  );
  ...
  const metrics = [
    { label: "Active Users", value: dashboardData.activeUsers, icon: <PersonOutlineIcon /> },
    { label: "Total Users", value: dashboardData.totalUsers, icon: <GroupAddOutlinedIcon /> },
    { label: "Active Providers", value: dashboardData.activeProviders, icon: <VerifiedOutlinedIcon /> },
    { label: "Unverified Providers", value: dashboardData.notVerifiedProviders, icon: <ReportGmailerrorredOutlinedIcon /> },
    { label: "Total Providers", value: dashboardData.totalProviders, icon: <WorkOutlineOutlinedIcon /> },
    { label: "Categories", value: dashboardData.totalCategories, icon: <CategoryOutlinedIcon /> },
  ];
      ...
      <Box sx={{ display: "flex" }}>
        ...
        <Box component="main" sx={{ flexGrow: 1, p: 3, mt: 8,mb:6 }}>
          <Grid container spacing={2} mb={3}>
            {metrics.map((item) => (
              <Grid key={item.label} size={{xs:12,sm:6,md:4,lg:3}} sx={{display:'flex',justifyContent:'center'}}>
                <StatCard {...item} />
              </Grid>
            ))}
          </Grid>
          <ActivityCard activity={dashboardData.monthlyActivity} />
        </Box>
      </Box>
    </ThemeProvider>
  );
